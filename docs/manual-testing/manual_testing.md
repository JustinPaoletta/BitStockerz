...
| Strategy persistence, versioning, or rule schema | [PR #9 pre-merge checklist](./PRE_MERGE_CHECKLIST.md) |
...
## Section 11 – Strategy persistence, versioning, and rule schema (Sprints 2.1–2.2)

The canonical, self-contained steps are in
[PRE_MERGE_CHECKLIST.md](./PRE_MERGE_CHECKLIST.md). The commands below remain a
shorter regression reference; the pre-merge checklist is the only required
manual sign-off source.
...
STRATEGY_HTTP_CODE=$(curl -s -o /tmp/bitstockerz-strategy-create.json \
  ...
  -d "$(jq -cn --argjson definition "$STRATEGY_DEFINITION" '{
    ...
    description:"Sprint 2.1–2.2 persistence and schema check",
    asset_type:"CRYPTO",
    timeframe:"1h",
