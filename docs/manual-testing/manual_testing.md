...
| Strategy persistence/versioning | Section 11 in seed mode and MySQL mode |
...
## Section 11 – Strategy persistence and versioning (Sprint 2.1)
...
STRATEGY_HTTP_CODE=$(curl -s -o /tmp/bitstockerz-strategy-create.json \
  ...
  -d '{
    ...
    "description":"Sprint 2.1 persistence check",
    "asset_type":"CRYPTO",
    "timeframe":"1h",
    "definition":{"rules":[]}
  }')
...
curl -s -o /tmp/bitstockerz-strategy-duplicate.json -w '%{http_code}\n' \
  ...
  -H "Authorization: Bearer $TOKEN" \
  -H 'Content-Type: application/json' \
  -d '{"name":"manual momentum","asset_type":"CRYPTO","timeframe":"1d","definition":{}}'
...
curl -s -X POST http://localhost:4000/api/strategies \
  -H "Authorization: Bearer $TOKEN" \
  -H 'Content-Type: application/json' \
  -d '{"name":"Invalid Equity","asset_type":"EQUITY","timeframe":"1h","definition":{}}' | jq

# Missing bearer token.
curl -s -X POST http://localhost:4000/api/strategies \
  -H 'Content-Type: application/json' \
  -d '{"name":"Unauthenticated","asset_type":"CRYPTO","timeframe":"1d","definition":{}}' | jq
```
...
curl -s -X POST http://localhost:4000/api/strategies \
  -H "Authorization: Bearer $TOKEN" \
  -H 'Content-Type: application/json' \
  -d '{"name":123,"description":456,"asset_type":"CRYPTO","timeframe":"1h","definition":{}}' | jq
...
Expected: `404 NOT_FOUND`; the response does not reveal that another user's strategy exists.
...
| 6 | Read by another user | `404 NOT_FOUND` |
