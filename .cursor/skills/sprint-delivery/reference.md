...
| Persisted ownership + MySQL | Auth users/sessions/passkeys persist in MySQL and hydrate on startup; `AuthService.ensureUserPersisted` upserts the in-memory user row before paper-account or FK writes |
